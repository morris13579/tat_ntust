...
void main() {
  ...
  PlatformCookie cookie(String name, {bool? secure, bool? httpOnly}) =>
      PlatformCookie(
        name: name,
        value: '$name-value',
        secure: secure,
  ...
  test('改寫成網域 cookie，不是 host-only', () async {
    ...
    final moved = await CookieBridge.mirrorToDio(
      url: url,
      jar: jar,
      source: _FakeSource([cookie('AuthServer')]),
    );
  ...
  test('WebView 沒有 cookie 時回 0，而且不動 jar', () async {
    ...
    final kept = await jar.loadForRequest(Uri.parse(url));
  ...
  test('拿到新 cookie 時整批取代，舊的不留', () async {
    final jar = CookieJar();
    await jar.saveFromResponse(Uri.parse(url), [io.Cookie('stale', 'v')]);

    await CookieBridge.mirrorToDio(
      url: url,
      jar: jar,
      source: _FakeSource([cookie('fresh')]),
    );

    final names = (await jar.loadForRequest(Uri.parse(url))).map((c) => c.name).toSet();
