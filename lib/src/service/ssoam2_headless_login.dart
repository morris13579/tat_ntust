...
///
/// WebView 由 [HeadlessWebViewHost] 開：Flutter App 是 HeadlessInAppWebView，
/// 原生版是 Swift 的 WKWebView，兩邊跑的是這同一份流程。
///
...
class Ssoam2HeadlessLogin {
  ...
  static Future<Ssoam2HeadlessOutcome> attempt({
    ...
    Future<void> onLoadStop(WebViewSession web, String? url) async {
      if (completer.isCompleted) return;
      final uri = url == null ? null : Uri.tryParse(url);
      Log.d("[sso-headless] onLoadStop ${uri?.host}${uri?.path}");
      ...
      if (Ssoam2Login.isLoginPage(url)) {
        if (await Ssoam2Login.hasValidationError(web)) {
        ...
        if (outcome == Ssoam2LoginOutcome.turnstileTimeout) {
          ...
          Log.d("[sso-headless] turnstile 診斷 "
              "${await Ssoam2Login.diagnoseTurnstile(web)}");
        }
      ...
      if (!navigatedToForm) {
        ...
        await web.loadUrl(Ssoam2Login.loginPageUrl);
        return;
      }
    ...
    try {
      ...
      // 開根路徑才會讓站台自己決定要不要導走。
      final web = await HeadlessWebViewHost.instance.open(Ssoam2Login.rootUrl);
      session = web;
      subscription = web.events.listen((event) async {
        switch (event) {
          case WebViewLoadStop(:final url):
            await onLoadStop(web, url);
          case WebViewLoadError(:final description):
            Log.d("[sso-headless] 載入失敗 $description");
            finish(Ssoam2HeadlessOutcome.failed);
          case WebViewIntercepted() || WebViewDismissed():
            break;
        }
      });
